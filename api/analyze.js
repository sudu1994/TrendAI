...
必ず純粋なJSON形式のみで返答してください。余計な解説文などは一切不要です。`;
  ...
  // FIXED: Proper variable assignment for userPrompt
  const userPrompt = `
キーワード: "${keyword}"
トレンド: ${trend}
スコア: ${score}
...
以下のJSONを生成してください：
{
  ...
  "websiteHTML": "完全なHTML文書（<!DOCTYPE html>から</html>まで）。Tailwind CDNとAlpine.js CDNを使用すること。全てのボタン（Get Started等）がクリック時にモーダル表示やアラート等のアクションを実行するように<script>タグ内にJavaScriptを記述すること。外部JSファイルは使わず、1つのHTML内で完結させること。"
}
